"""
Lab 1 (Extra) - Neural Network, for Gene's Data (Dataset 1)
...
# Retrieve the ys (Labels) of the Data of the Genes (Dataset 1)
ys_data_labels = matrix_data[:, -1]

# Retrieve the xs (Features) of the Data of the Genes (Dataset 1)
...
# from a given set of xs (Features) of the Data of the Genes (Dataset 1),
# using the configured Artificial Neural Network (ANN)
def neural_network_prediction(xs_data_features_to_predict):

    # Create a TensorFlow Constant from the given set of xs (Features) of the Data of the Genes (Dataset 1)
    tensorflow_constant_xs_data_features_to_predict = \
        tensorflow.constant(xs_data_features_to_predict.astype(numpy.float32))

    # Sum the xs (Features) of the Data of the Genes (Dataset 1) weighted by the neurons (features x weights),
...
def compute_gradient(xs_data_features_to_predict, ys_real_data_labels, weights_neurons, bias):
    ...
    with tensorflow.GradientTape() as tape:
        ...
        # the Genes (Dataset 1) given as arguments of the Artificial Neural Network (ANN)
...
# with a Learning Rate of 10% and momentum of 90%
stochastic_gradient_descent_optimizer = tensorflow.optimizers.SGD(learning_rate=0.1, momentum=0.9)
...
def execute_artificial_neural_network():
    ...
    for current_epoch in range(num_epochs):
        ...
        # Shuffle the ys (Labels) for the Data of the Genes (Dataset 1)
        ...
        for current_num_batch in range(num_batches_per_epoch):
            ...
            # Retrieve the chosen Samples from the xs (Features) of the Data of the Genes (Dataset 1)
            batch_xs_data_features = \
                xs_data_features_standardized[ys_data_labels_shuffled[start_num_sample:
                                                                      (start_num_sample + batch_size)], :]

            # Retrieve the chosen Samples from the ys (Labels) of the Data of the Genes (Dataset 1)
            ...
            # the chosen Samples from the xs (Features) and ys (Labels) of the Data of the Genes (Dataset 1),
        ...
        # xs (Features) of the Data of the Genes (Dataset 1), using the configured Artificial Neural Network (ANN)
        ys_labels_predicted_for_data = neural_network_prediction(xs_data_features_standardized)

        # Compute the Cost of the Logistic Loss, between the predicted ys (Labels) of the Data of the Genes (Dataset 1),
        # through the Artificial Neural Network (ANN) and the real ys (Labels) of the Data of the Genes (Dataset 1)
...
print("\n\nStart the execution of the Artificial Neural Network (ANN), with {} Epochs, Bath Size of {},\n"
      "with the Learning Algorithm of Stochastic Gradient Descent (SDG), for the Dataset of the Genes (Dataset 1)...\n"
      .format(num_epochs, batch_size))

# Start the execution of the Artificial Neural Network (ANN),
# for the the Prediction of the Data of the Genes (Dataset 1)
