"""
Lab 1 (Extra) - Neural Network, for Gene's Data (Dataset 3)
...
matrix_data = numpy.loadtxt("files/gene_data_3.txt", delimiter="\t")

# Retrieve the ys (Labels) of the Data of the Genes (Dataset 3)
ys_data_labels = matrix_data[:, -1]

# Retrieve the xs (Features) of the Data of the Genes (Dataset 3)
...
# from a given set of xs (Features) of the Data of the Genes (Dataset 3),
# using the configured Artificial Neural Network (ANN)
def neural_network_prediction(xs_data_features_to_predict):

    # Create a TensorFlow Constant from the given set of xs (Features) of the Data of the Genes (Dataset 3)
...
def compute_gradient(xs_data_features_to_predict, ys_real_data_labels, weights_neurons, bias):
    ...
    with tensorflow.GradientTape() as tape:
        ...
        # the Genes (Dataset 3) given as arguments of the Artificial Neural Network (ANN)
...
stochastic_gradient_descent_optimizer = tensorflow.optimizers.SGD(learning_rate=0.05)
...
def execute_artificial_neural_network():
    ...
    for current_epoch in range(num_epochs):
        ...
        # Shuffle the ys (Labels) for the Data of the Genes (Dataset 3)
        ...
        for current_num_batch in range(num_batches_per_epoch):
            ...
            # Retrieve the chosen Samples from the xs (Features) of the Data of the Genes (Dataset 3)
            batch_xs_data_features = \
                xs_data_features_standardized[ys_data_labels_shuffled[start_num_sample:
                                                                      (start_num_sample + batch_size)], :]

            # Retrieve the chosen Samples from the ys (Labels) of the Data of the Genes (Dataset 3)
        ...
        # xs (Features) of the Data of the Genes (Dataset 3), using the configured Artificial Neural Network (ANN)
        ys_labels_predicted_for_data = neural_network_prediction(xs_data_features_standardized)

        # Compute the Cost of the Logistic Loss, between the predicted ys (Labels) of the Data of the Genes (Dataset 3),
        # through the Artificial Neural Network (ANN) and the real ys (Labels) of the Data of the Genes (Dataset 3)
...
print("\n\nStart the execution of the Artificial Neural Network (ANN), with {} Epochs, Bath Size of {},\n"
      "with the Learning Algorithm of Stochastic Gradient Descent (SDG), for the Dataset of the Genes (Dataset 3)...\n"
      .format(num_epochs, batch_size))

# Start the execution of the Artificial Neural Network (ANN),
# for the the Prediction of the Data of the Genes (Dataset 3)
